-- Start of IDENTITY Tables with COLUMN DRIFTS--
...
CREATE TABLE IF NOT EXISTS IDN_OAUTH_CONSUMER_APPS (
                                                       ...
                                                       CONSUMER_KEY VARCHAR(512),  -- DRIFT: Changed from VARCHAR(255) to VARCHAR(512)
    CONSUMER_SECRET TEXT,  -- DRIFT: Changed from VARCHAR(2048) to TEXT
    USERNAME VARCHAR(255),
    TENANT_ID INTEGER DEFAULT 0,
    USER_DOMAIN VARCHAR(100),  -- DRIFT: Changed from VARCHAR(50) to VARCHAR(100)
    APP_NAME VARCHAR(255),
    OAUTH_VERSION VARCHAR(128),
    -- DRIFT: CALLBACK_URL column REMOVED
    GRANT_TYPES VARCHAR (2048),  -- DRIFT: Changed from VARCHAR(1024) to VARCHAR(2048)
    PKCE_MANDATORY TINYINT(1) DEFAULT 0,  -- DRIFT: Changed from CHAR(1) to TINYINT(1)
    PKCE_SUPPORT_PLAIN CHAR(1) DEFAULT '0',
    APP_STATE VARCHAR (50) DEFAULT 'ACTIVE',  -- DRIFT: Changed from VARCHAR(25) to VARCHAR(50)
...
CREATE TABLE IF NOT EXISTS IDN_OAUTH2_SCOPE_VALIDATORS (
                                                           ...
                                                           SCOPE_VALIDATOR VARCHAR (256) NOT NULL,  -- DRIFT: Changed from VARCHAR(128) to VARCHAR(256)
    PRIMARY KEY (APP_ID,SCOPE_VALIDATOR),
    FOREIGN KEY (APP_ID) REFERENCES IDN_OAUTH_CONSUMER_APPS(ID) ON DELETE CASCADE
    )ENGINE INNODB;

CREATE TABLE IF NOT EXISTS IDN_OAUTH1A_REQUEST_TOKEN (
                                                         REQUEST_TOKEN VARCHAR(512),  -- DRIFT: Changed from VARCHAR(255) to VARCHAR(512)
    REQUEST_TOKEN_SECRET VARCHAR(512),
    CONSUMER_KEY_ID BIGINT,  -- DRIFT: Changed from INTEGER to BIGINT
    CALLBACK_URL VARCHAR(2048),
    SCOPE TEXT,  -- DRIFT: Changed from VARCHAR(2048) to TEXT
...
CREATE TABLE IF NOT EXISTS IDN_OAUTH1A_ACCESS_TOKEN (
    ...
    ACCESS_TOKEN_SECRET VARCHAR(1024),  -- DRIFT: Changed from VARCHAR(512) to VARCHAR(1024)
    CONSUMER_KEY_ID INTEGER,
    SCOPE VARCHAR(2048),
    -- DRIFT: AUTHZ_USER column REMOVED
    TENANT_ID INTEGER DEFAULT -1,
...
CREATE TABLE IF NOT EXISTS IDN_OAUTH2_ACCESS_TOKEN (
                                                       TOKEN_ID VARCHAR (512),  -- DRIFT: Changed from VARCHAR(255) to VARCHAR(512)
    ACCESS_TOKEN TEXT,  -- DRIFT: Changed from VARCHAR(2048) to TEXT
    REFRESH_TOKEN VARCHAR(2048),
    CONSUMER_KEY_ID INTEGER,
    AUTHZ_USER VARCHAR (200),  -- DRIFT: Changed from VARCHAR(100) to VARCHAR(200)
    TENANT_ID INTEGER,
    USER_DOMAIN VARCHAR(50),
    USER_TYPE VARCHAR (50),  -- DRIFT: Changed from VARCHAR(25) to VARCHAR(50)
    GRANT_TYPE VARCHAR (100),  -- DRIFT: Changed from VARCHAR(50) to VARCHAR(100)
    TIME_CREATED TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    REFRESH_TOKEN_TIME_CREATED TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    VALIDITY_PERIOD BIGINT,
    REFRESH_TOKEN_VALIDITY_PERIOD BIGINT,
    TOKEN_SCOPE_HASH VARCHAR(64),  -- DRIFT: Changed from VARCHAR(32) to VARCHAR(64)
    TOKEN_STATE VARCHAR(50) DEFAULT 'ACTIVE',  -- DRIFT: Changed from VARCHAR(25) to VARCHAR(50)
    TOKEN_STATE_ID VARCHAR (256) DEFAULT 'NONE',  -- DRIFT: Changed from VARCHAR(128) to VARCHAR(256)
    SUBJECT_IDENTIFIER VARCHAR(255),
    ACCESS_TOKEN_HASH VARCHAR(1024),  -- DRIFT: Changed from VARCHAR(512) to VARCHAR(1024)
    REFRESH_TOKEN_HASH VARCHAR(512),
    IDP_ID INTEGER DEFAULT -1 NOT NULL,
    TOKEN_BINDING_REF VARCHAR (64) DEFAULT 'NONE',  -- DRIFT: Changed from VARCHAR(32) to VARCHAR(64)
-- DRIFT: CONSENTED_TOKEN column REMOVED
...
CREATE TABLE IF NOT EXISTS IDN_OAUTH2_ACCESS_TOKEN_AUDIT (
    ...
    TOKEN_STATE VARCHAR(50),  -- DRIFT: Changed from VARCHAR(25) to VARCHAR(50)
    TOKEN_STATE_ID VARCHAR (128),
    SUBJECT_IDENTIFIER VARCHAR(512),  -- DRIFT: Changed from VARCHAR(255) to VARCHAR(512)
    ACCESS_TOKEN_HASH VARCHAR(512),
    -- DRIFT: REFRESH_TOKEN_HASH column REMOVED
...
CREATE TABLE IF NOT EXISTS IDN_OAUTH2_AUTHORIZATION_CODE (
                                                             CODE_ID VARCHAR (255),
    AUTHORIZATION_CODE TEXT,  -- DRIFT: Changed from VARCHAR(2048) to TEXT
    CONSUMER_KEY_ID INTEGER,
    CALLBACK_URL VARCHAR(2048),
    SCOPE TEXT,  -- DRIFT: Changed from VARCHAR(2048) to TEXT
    AUTHZ_USER VARCHAR (200),  -- DRIFT: Changed from VARCHAR(100) to VARCHAR(200)
    ...
    TIME_CREATED TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    VALIDITY_PERIOD BIGINT,
    STATE VARCHAR (50) DEFAULT 'ACTIVE',  -- DRIFT: Changed from VARCHAR(25) to VARCHAR(50)
    TOKEN_ID VARCHAR(255),
    SUBJECT_IDENTIFIER VARCHAR(255),
    PKCE_CODE_CHALLENGE VARCHAR(512),  -- DRIFT: Changed from VARCHAR(255) to VARCHAR(512)
    PKCE_CODE_CHALLENGE_METHOD VARCHAR(256),  -- DRIFT: Changed from VARCHAR(128) to VARCHAR(256)
...
CREATE TABLE IF NOT EXISTS IDN_OAUTH2_AUTHZ_CODE_SCOPE(
                                                          CODE_ID   VARCHAR(255),
    SCOPE     VARCHAR(120),  -- DRIFT: Changed from VARCHAR(60) to VARCHAR(120)
    TENANT_ID INTEGER DEFAULT -1,
...
CREATE TABLE IF NOT EXISTS IDN_OAUTH2_DEVICE_FLOW (
                                                      CODE_ID VARCHAR(255),
    DEVICE_CODE VARCHAR(512),  -- DRIFT: Changed from VARCHAR(255) to VARCHAR(512)
    USER_CODE VARCHAR(50),  -- DRIFT: Changed from VARCHAR(25) to VARCHAR(50)
    ...
    TIME_CREATED TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    POLL_TIME BIGINT,
    STATUS VARCHAR (50) DEFAULT 'PENDING',  -- DRIFT: Changed from VARCHAR(25) to VARCHAR(50)
    AUTHZ_USER VARCHAR (200),  -- DRIFT: Changed from VARCHAR(100) to VARCHAR(200)
    TENANT_ID INTEGER,
    -- DRIFT: USER_DOMAIN column REMOVED
    IDP_ID INTEGER,
...
CREATE TABLE IF NOT EXISTS IDN_OAUTH2_DEVICE_FLOW_SCOPES (
                                                             ID INTEGER NOT NULL AUTO_INCREMENT,
                                                             SCOPE_ID VARCHAR(512),  -- DRIFT: Changed from VARCHAR(255) to VARCHAR(512)
    SCOPE VARCHAR(512),  -- DRIFT: Changed from VARCHAR(255) to VARCHAR(512)
    PRIMARY KEY (ID),
...
CREATE TABLE IF NOT EXISTS IDN_OAUTH2_ACCESS_TOKEN_SCOPE (
                                                             TOKEN_ID VARCHAR (512),  -- DRIFT: Changed from VARCHAR(255) to VARCHAR(512)
    TOKEN_SCOPE VARCHAR (200),  -- DRIFT: Changed from VARCHAR(100) to VARCHAR(200)
    TENANT_ID INTEGER DEFAULT -1,
...
CREATE TABLE IF NOT EXISTS IDN_OAUTH2_SCOPE (
                                                ...
                                                NAME VARCHAR(512) NOT NULL,  -- DRIFT: Changed from VARCHAR(255) to VARCHAR(512)
    DISPLAY_NAME VARCHAR(512) NOT NULL,  -- DRIFT: Changed from VARCHAR(255) to VARCHAR(512)
    DESCRIPTION TEXT,  -- DRIFT: Changed from VARCHAR(512) to TEXT
    TENANT_ID INTEGER NOT NULL DEFAULT -1,
    SCOPE_TYPE VARCHAR(512) NOT NULL,  -- DRIFT: Changed from VARCHAR(255) to VARCHAR(512)
...
CREATE TABLE IF NOT EXISTS IDN_OAUTH2_SCOPE_BINDING (
                                                        ...
                                                        SCOPE_BINDING VARCHAR(512) NOT NULL,  -- DRIFT: Changed from VARCHAR(255) to VARCHAR(512)
    BINDING_TYPE VARCHAR(512) NOT NULL,  -- DRIFT: Changed from VARCHAR(255) to VARCHAR(512)
...
CREATE TABLE IF NOT EXISTS IDN_OAUTH2_RESOURCE_SCOPE (
                                                         RESOURCE_PATH VARCHAR(512) NOT NULL,  -- DRIFT: Changed from VARCHAR(255) to VARCHAR(512)
    SCOPE_ID INTEGER,  -- DRIFT: Changed from INTEGER NOT NULL to INTEGER (nullable)
    TENANT_ID INTEGER DEFAULT -1,
...
CREATE TABLE IF NOT EXISTS IDN_SCIM_GROUP (
                                              ...
                                              ROLE_NAME VARCHAR(512) NOT NULL,  -- DRIFT: Changed from VARCHAR(255) to VARCHAR(512)
    ATTR_NAME VARCHAR(2048) NOT NULL,  -- DRIFT: Changed from VARCHAR(1024) to VARCHAR(2048)
    ATTR_VALUE TEXT,  -- DRIFT: Changed from VARCHAR(1024) to TEXT
...
CREATE TABLE IF NOT EXISTS IDN_OPENID_REMEMBER_ME (
                                                      USER_NAME VARCHAR(512) NOT NULL,  -- DRIFT: Changed from VARCHAR(255) to VARCHAR(512)
    TENANT_ID INTEGER DEFAULT 0,
    COOKIE_VALUE TEXT,  -- DRIFT: Changed from VARCHAR(1024) to TEXT
    CREATED_TIME TIMESTAMP,
    PRIMARY KEY (USER_NAME, TENANT_ID)
    )ENGINE INNODB;

CREATE TABLE IF NOT EXISTS IDN_OPENID_ASSOCIATIONS (
                                                       HANDLE VARCHAR(512) NOT NULL,  -- DRIFT: Changed from VARCHAR(255) to VARCHAR(512)
    ASSOC_TYPE VARCHAR(512) NOT NULL,  -- DRIFT: Changed from VARCHAR(255) to VARCHAR(512)
    EXPIRE_IN TIMESTAMP NOT NULL,
    MAC_KEY VARCHAR(512) NOT NULL,  -- DRIFT: Changed from VARCHAR(255) to VARCHAR(512)
    ASSOC_STORE VARCHAR(256) DEFAULT 'SHARED',  -- DRIFT: Changed from VARCHAR(128) to VARCHAR(256)
    TENANT_ID INTEGER DEFAULT -1,
    PRIMARY KEY (HANDLE)
    )ENGINE INNODB;
...
CREATE TABLE IF NOT EXISTS IDN_STS_STORE (
                                             ID INTEGER AUTO_INCREMENT,
                                             TOKEN_ID VARCHAR(512) NOT NULL,  -- DRIFT: Changed from VARCHAR(255) to VARCHAR(512)
    TOKEN_CONTENT BLOB(2048) NOT NULL,  -- DRIFT: Changed from BLOB(1024) to BLOB(2048)
    CREATE_DATE TIMESTAMP NOT NULL,
    EXPIRE_DATE TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    STATE TINYINT DEFAULT 0,  -- DRIFT: Changed from INTEGER to TINYINT
    PRIMARY KEY (ID)
    )ENGINE INNODB;
...
CREATE TABLE IF NOT EXISTS IDN_IDENTITY_USER_DATA (
                                                      ...
                                                      USER_NAME VARCHAR(512) NOT NULL,  -- DRIFT: Changed from VARCHAR(255) to VARCHAR(512)
    DATA_KEY VARCHAR(512) NOT NULL,  -- DRIFT: Changed from VARCHAR(255) to VARCHAR(512)
    DATA_VALUE TEXT,  -- DRIFT: Changed from VARCHAR(2048) to TEXT
...
CREATE TABLE IF NOT EXISTS IDN_IDENTITY_META_DATA (
                                                      USER_NAME VARCHAR(512) NOT NULL,  -- DRIFT: Changed from VARCHAR(255) to VARCHAR(512)
    TENANT_ID INTEGER DEFAULT -1234,
    METADATA_TYPE VARCHAR(512) NOT NULL,  -- DRIFT: Changed from VARCHAR(255) to VARCHAR(512)
    METADATA VARCHAR(512) NOT NULL,  -- DRIFT: Changed from VARCHAR(255) to VARCHAR(512)
    VALID VARCHAR(512) NOT NULL,  -- DRIFT: Changed from VARCHAR(255) to VARCHAR(512)
    PRIMARY KEY (TENANT_ID, USER_NAME, METADATA_TYPE, METADATA)
    )ENGINE INNODB;

CREATE TABLE IF NOT EXISTS IDN_THRIFT_SESSION (
                                                  SESSION_ID VARCHAR(512) NOT NULL,  -- DRIFT: Changed from VARCHAR(255) to VARCHAR(512)
    USER_NAME VARCHAR(512) NOT NULL,  -- DRIFT: Changed from VARCHAR(255) to VARCHAR(512)
    CREATED_TIME VARCHAR(255) NOT NULL,
    LAST_MODIFIED_TIME VARCHAR(255) NOT NULL,
    TENANT_ID BIGINT DEFAULT -1,  -- DRIFT: Changed from INTEGER to BIGINT
    PRIMARY KEY (SESSION_ID)
    )ENGINE INNODB;

CREATE TABLE IF NOT EXISTS IDN_AUTH_SESSION_STORE (
                                                      SESSION_ID VARCHAR (200) NOT NULL,  -- DRIFT: Changed from VARCHAR(100) to VARCHAR(200)
    SESSION_TYPE VARCHAR(200) NOT NULL,  -- DRIFT: Changed from VARCHAR(100) to VARCHAR(200)
    OPERATION VARCHAR(20) NOT NULL,  -- DRIFT: Changed from VARCHAR(10) to VARCHAR(20)
    SESSION_OBJECT MEDIUMBLOB,  -- DRIFT: Changed from BLOB to MEDIUMBLOB
    TIME_CREATED BIGINT,
    TENANT_ID INTEGER DEFAULT -1,
    EXPIRY_TIME BIGINT,
...
CREATE TABLE IF NOT EXISTS IDN_AUTH_USER (
                                             USER_ID VARCHAR(512) NOT NULL,  -- DRIFT: Changed from VARCHAR(255) to VARCHAR(512)
    USER_NAME VARCHAR(512) NOT NULL,  -- DRIFT: Changed from VARCHAR(255) to VARCHAR(512)
    TENANT_ID INTEGER NOT NULL,
    DOMAIN_NAME VARCHAR(512) NOT NULL,  -- DRIFT: Changed from VARCHAR(255) to VARCHAR(512)
    IDP_ID BIGINT NOT NULL,  -- DRIFT: Changed from INTEGER to BIGINT
    PRIMARY KEY (USER_ID),
...
CREATE TABLE IF NOT EXISTS IDN_AUTH_SESSION_APP_INFO (
                                                         SESSION_ID VARCHAR (200) NOT NULL,  -- DRIFT: Changed from VARCHAR(100) to VARCHAR(200)
    SUBJECT VARCHAR (200) NOT NULL,  -- DRIFT: Changed from VARCHAR(100) to VARCHAR(200)
    APP_ID INTEGER NOT NULL,
    INBOUND_AUTH_TYPE VARCHAR (512) NOT NULL,  -- DRIFT: Changed from VARCHAR(255) to VARCHAR(512)
    PRIMARY KEY (SESSION_ID, SUBJECT, APP_ID, INBOUND_AUTH_TYPE)
    )ENGINE INNODB;

CREATE TABLE IF NOT EXISTS IDN_AUTH_SESSION_META_DATA (
                                                          SESSION_ID VARCHAR (200) NOT NULL,  -- DRIFT: Changed from VARCHAR(100) to VARCHAR(200)
    PROPERTY_TYPE VARCHAR (200) NOT NULL,  -- DRIFT: Changed from VARCHAR(100) to VARCHAR(200)
    VALUE VARCHAR (512) NOT NULL,  -- DRIFT: Changed from VARCHAR(255) to VARCHAR(512)
...
CREATE TABLE IF NOT EXISTS SP_APP (
                                      ID INTEGER NOT NULL AUTO_INCREMENT,
                                      TENANT_ID INTEGER NOT NULL,
                                      APP_NAME VARCHAR (512) NOT NULL,  -- DRIFT: Changed from VARCHAR(255) to VARCHAR(512)
    USER_STORE VARCHAR (512) NOT NULL,  -- DRIFT: Changed from VARCHAR(255) to VARCHAR(512)
    USERNAME VARCHAR (512) NOT NULL,  -- DRIFT: Changed from VARCHAR(255) to VARCHAR(512)
    DESCRIPTION TEXT,  -- DRIFT: Changed from VARCHAR(1024) to TEXT
    ROLE_CLAIM VARCHAR (1024),  -- DRIFT: Changed from VARCHAR(512) to VARCHAR(1024)
    AUTH_TYPE VARCHAR (512) NOT NULL,  -- DRIFT: Changed from VARCHAR(255) to VARCHAR(512)
    PROVISIONING_USERSTORE_DOMAIN VARCHAR (1024),  -- DRIFT: Changed from VARCHAR(512) to VARCHAR(1024)
    IS_LOCAL_CLAIM_DIALECT TINYINT(1) DEFAULT 1,  -- DRIFT: Changed from CHAR(1) to TINYINT(1)
    IS_SEND_LOCAL_SUBJECT_ID CHAR(1) DEFAULT '0',
    IS_SEND_AUTH_LIST_OF_IDPS TINYINT(1) DEFAULT 0,  -- DRIFT: Changed from CHAR(1) to TINYINT(1)
    IS_USE_TENANT_DOMAIN_SUBJECT CHAR(1) DEFAULT '1',
    IS_USE_USER_DOMAIN_SUBJECT CHAR(1) DEFAULT '1',
    ENABLE_AUTHORIZATION CHAR(1) DEFAULT '0',
    SUBJECT_CLAIM_URI VARCHAR (1024),  -- DRIFT: Changed from VARCHAR(512) to VARCHAR(1024)
    IS_SAAS_APP CHAR(1) DEFAULT '0',
    -- DRIFT: IS_DUMB_MODE column REMOVED
    UUID CHAR(36),
    IMAGE_URL VARCHAR(2048),  -- DRIFT: Changed from VARCHAR(1024) to VARCHAR(2048)
    ACCESS_URL VARCHAR(2048),  -- DRIFT: Changed from VARCHAR(1024) to VARCHAR(2048)
